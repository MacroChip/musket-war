...
**+ AI medic** buttons (dismiss one with ✕), and **⚖ Even the sides with AI**
fills the smaller team with AI infantry until the rolls match. AI soldiers
are always ready, so a single human can drill against a full enemy line. The
server plays their hand: they hold the line proactively (each man dresses on
his neighbors, so he advances and gives ground as the men beside him move,
and closes up at a run when a tether stretches), volley when loaded, take a
believably long time over the reload ritual, revive the fallen, flee and lay
bear traps when routed, and steer the bayonet charge.
